...
@interface JXShopDetailTabScrollView ()<LYSelectTabBarDelegate>
...
@property (nonatomic,strong) JXShopDetailTabScrollViewModel *viewModel;

@end

@implementation JXShopDetailTabScrollView

- (instancetype)initWithShopId:(long)shopId{
    
    self = [super init];
    if(self){
        self.viewModel = [[JXShopDetailTabScrollViewModel alloc]init];
        NSMutableArray *subViews = [NSMutableArray array];
        
        NSInteger count = [self.viewModel tabBarTitleArray].count;
        @weakify(self)
        for(NSInteger i=0;i<count;i++){
            JXShopDetailCollectionView *view = [[JXShopDetailCollectionView alloc]initWithLoadMoreBlock:^{
                @strongify(self)
        ...
        _scrollView = [[JXShopDetailScrollView alloc]initWithSubViews:subViews];
...
- (JXShopDetailCollectionView *)currentCollectionView{
    return (JXShopDetailCollectionView *)self.scrollView.subViews[_currentIndex];
}


#pragma mark --- bind data-----
- (void)bindData:(id)data atIndex:(NSInteger)index{
    JXShopDetailCollectionView *view = (JXShopDetailCollectionView *)[self.scrollView subScorllViewAtIndex:index];
