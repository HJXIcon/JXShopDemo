...
@interface JXShopDetailViewModel ()

@property (nonatomic,strong) JXShopDetailUserCase *userCase;

@end
@implementation JXShopDetailViewModel
...
- (JXShopDetailUserCase *)userCase{
    if(!_userCase){
        _userCase = [[JXShopDetailUserCase alloc]init];
    }
    return _userCase;
}
...
- (void)requestShopDetail{
    [self.userCase shopInfoRequestWithShopId:_shopId Success:^(JXShopDetailInfo *info) {
        self.shopInfo = info;
    } fail:^(NSString *errorMsg) {
